resource "oci_vault_secret" "vault_secret" {
  ...
  dynamic "secret_content" {
    for_each = each.value.is_vault_secret_secret_content_definded ? [1] : []
    content {
  ...
  dynamic "secret_generation_context" {
    for_each = each.value.is_vault_secret_secret_generation_context_definded ? [1] : []
    content {
  ...
  dynamic "secret_rules" {
    for_each = each.value.is_vault_secret_secret_rules_definded ? [1] : []
    content {
